    ...
    <div v-if="unfold" class="space-y-1 pl-0 py-1 pr-2">
      ...
      >
        <ViewerSelectionObject
          ...
          :unfold="autoUnfoldKeys.includes(kvp.key) || shouldForceExpand"
      ...
      >
        <button
          type="button"
          class="grid w-full grid-cols-3 pl-1 pr-2 py-1 text-left rounded-[2px] bg-foundation-2 hover:bg-highlight-1"
          @click="toggleArrayExpand(kvp.key as string)"
        >
          <div
            class="col-span-1 flex items-center gap-1 truncate text-body-3xs font-medium text-foreground-2"
            :title="(kvp.key as string)"
          >
            <IconTriangle
              :class="`h-3 w-3 shrink-0 ${isArrayExpanded(kvp.key as string) ? 'rotate-90' : ''}`"
            />
            {{ kvp.key }}
          </div>
          <div
        ...
        <div v-if="isArrayExpanded(kvp.key as string)" class="space-y-1 pl-2 pt-1">
          <template v-if="kvp.innerType === 'object'">
            <ViewerSelectionObject
              v-for="(item, itemIndex) in kvp.value as SpeckleObject[]"
              :key="`${kvp.key}-${itemIndex}`"
              :object="item || {}"
              :title="`${kvp.key}[${itemIndex}]`"
              :parent-path="arrayItemPath(kvp.key as string, itemIndex)"
              :unfold="shouldForceExpand"
            />
          </template>
          <template v-else-if="kvp.innerType === 'array'">
            <ViewerSelectionObject
              v-for="(item, itemIndex) in kvp.value as unknown[]"
              :key="`${kvp.key}-${itemIndex}`"
              :object="{ items: item }"
              :title="`${kvp.key}[${itemIndex}]`"
              :parent-path="arrayItemPath(kvp.key as string, itemIndex)"
...
import {
  SELECTION_PANEL_EXPAND_KEY,
  type SelectionPanelExpandControl
} from '~/components/viewer/selection/panelExpand'
...
const expandedArrayKeys = ref<Set<string>>(new Set())

const panelExpandControl = inject<SelectionPanelExpandControl | null>(
  SELECTION_PANEL_EXPAND_KEY,
  null
)

const shouldForceExpand = computed(() => {
  if (!panelExpandControl) return false
  return panelExpandControl.expandAllTick.value > panelExpandControl.collapseAllTick.value
})

const isArrayExpanded = (key: string) => {
  if (shouldForceExpand.value) return true
  return expandedArrayKeys.value.has(key)
}

const toggleArrayExpand = (key: string) => {
  const next = new Set(expandedArrayKeys.value)
  if (next.has(key)) next.delete(key)
  else next.add(key)
  expandedArrayKeys.value = next
}

const arrayItemPath = (key: string, index: number) => {
  const base = currentPath.value ? `${currentPath.value}.${key}` : key
  return `${base}[${index}]`
}
...
watch(
  () => panelExpandControl?.expandAllTick.value,
  () => {
    if (shouldForceExpand.value) unfold.value = true
  }
)

watch(
  () => panelExpandControl?.collapseAllTick.value,
  () => {
    if (!shouldForceExpand.value) {
      unfold.value = false
      expandedArrayKeys.value = new Set()
    }
  }
)
</script>
