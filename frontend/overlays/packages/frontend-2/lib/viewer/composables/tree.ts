...
const EXCLUDED_COLLECTION_KEYS = new Set(['children', 'elements', '@elements'] as const)
...
export function useTreeManagement() {
  ...
  const flattenModelTree = (
    ...
    for (const node of nodes) {
      ...
      const worldTreeChildren =
        node.children?.filter((child) => !!child.raw?.id && isAllowedType(child)) || []
      const hasWorldTreeChildren = worldTreeChildren.length > 0
      ...
      const hasChildren = isSingleCollection || isMultipleCollection || hasWorldTreeChildren
