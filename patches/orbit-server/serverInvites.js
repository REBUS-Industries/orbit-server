...
import { InviteCreateValidationError, InviteNotFoundError } from '@/modules/serverinvites/errors';
import { adminOverrideEnabled } from '@/modules/shared/helpers/envHelper';
import { createAndSendInviteFactory, resendInviteEmailFactory } from '@/modules/serverinvites/services/creation';
import { cancelResourceInviteFactory, deleteInviteFactory, finalizeInvitedServerRegistrationFactory, finalizeResourceInviteFactory } from '@/modules/serverinvites/services/processing';
import { getInvitationTargetUsersFactory, getServerInviteForTokenFactory } from '@/modules/serverinvites/services/retrieval';
import { authorizeResolver } from '@/modules/shared';
import { chunk } from 'lodash-es';
import { TokenResourceIdentifierType } from '@/modules/core/graph/generated/graphql';
import db from '@/db/knex';
import { deleteInvitesByTargetFactory, findInviteFactory, findServerInviteFactory, findUserByTargetFactory, insertInviteAndDeleteOldFactory, deleteInviteFactory as deleteInviteFromDbFactory, queryAllUserResourceInvitesFactory, queryAllResourceInvitesFactory, markInviteUpdatedFactory, deleteServerOnlyInvitesFactory, updateAllInviteTargetsFactory } from '@/modules/serverinvites/repositories/serverInvites';
import { createProjectInviteFactory, getPendingProjectCollaboratorsFactory, getUserPendingProjectInviteFactory, getUserPendingProjectInvitesFactory, useProjectInviteAndNotifyFactory } from '@/modules/serverinvites/services/projectInviteManagement';
import { collectAndValidateCoreTargetsFactory } from '@/modules/serverinvites/services/coreResourceCollection';
import { buildCoreInviteEmailContentsFactory } from '@/modules/serverinvites/services/coreEmailContents';
import { getEventBus } from '@/modules/shared/services/eventBus';
import { ServerInviteLimit, ServerInviteResourceType } from '@/modules/serverinvites/domain/constants';
import { processFinalizedProjectInviteFactory, validateProjectInviteBeforeFinalizationFactory } from '@/modules/serverinvites/services/coreFinalization';
import { createUserEmailFactory, ensureNoPrimaryEmailForUserFactory, findEmailFactory } from '@/modules/core/repositories/userEmails';
import { validateAndCreateUserEmailFactory } from '@/modules/core/services/userEmails';
import { requestNewEmailVerificationFactory } from '@/modules/emails/services/verification/request';
import { deleteOldAndInsertNewVerificationFactory } from '@/modules/emails/repositories';
import { renderEmail } from '@/modules/emails/services/emailRendering';
import { sendEmail } from '@/modules/emails/services/sending';
import { getStreamFactory, getStreamRolesFactory, grantStreamPermissionsFactory } from '@/modules/core/repositories/streams';
...
// ── ORBIT patch ──────────────────────────────────────────────────────────────
// Treat InviteNotFoundError as a benign duplicate when the user already
// has access to the project. The Speckle frontend fires multiple
// `useProjectInvite` mutations per Accept click (Apollo refetch racing
// with the user click); the first call deletes the invite row and adds
// the user to stream_acl, and the duplicates land on an empty row and
// throw "Attempted to finalize nonexistant invite". Without this guard,
// the duplicate error bubbles up as the user-visible "Couldn't process
// project invite" toast even though the membership grant succeeded.
//
// We swallow ONLY when:
//   - the thrown error is InviteNotFoundError (don't mask other failures)
//   - AND the calling user does already have effective access to the
//     project, either as an explicit stream_acl member or as a server
//     admin (when ADMIN_OVERRIDE_ENABLED is on — see streams.js patch).
// Any other failure mode (truly bogus token, mismatched email, processInvite
// error) re-throws normally so legitimate problems still surface.
async function isHarmlessDuplicateUseError(err, ctx, projectId) {
    if (!(err instanceof InviteNotFoundError)) return false;
    if (!projectId || !ctx.userId) return false;
    try {
        if (ctx.role === Roles.Server.Admin && adminOverrideEnabled()) return true;
        const project = await getStream({ streamId: projectId, userId: ctx.userId });
        return !!project?.role;
    } catch {
        return false;
    }
...
const buildCollectAndValidateResourceTargets = () => collectAndValidateCoreTargetsFactory({
    getStream
});
const buildFinalizeProjectInvite = () => finalizeResourceInviteFactory({
...
const buildCreateAndSendServerOrProjectInvite = () => createAndSendInviteFactory({
    ...
    emitEvent: ({ eventName, payload }) => getEventBus().emit({
        eventName,
        payload
    }),
    getUser,
...
export default {
    ...
    Mutation: {
        async serverInviteCreate(_parent, args, context) {
            ...
            const primaryResourceTarget = {
                resourceId: '',
                role: args.input.serverRole || Roles.Server.User,
                resourceType: ServerInviteResourceType,
                primary: true
            };
            await withOperationLogging(async () => await createAndSendInvite({
                target: targetEmail,
                inviterId,
        ...
        async streamInviteCreate(_parent, args, context) {
            ...
            await withOperationLogging(async () => await createProjectInvite({
                input: args.input,
        ...
        async serverInviteBatchCreate(_parent, args, context) {
            ...
            if (inviteCount > ServerInviteLimit && context.role !== Roles.Server.Admin) {
                throw new InviteCreateValidationError(`Maximum ${ServerInviteLimit} invites can be sent at once by non admins`);
            }
            const logger = context.log.child({
                inviteCount
            });
            ...
            for (const paramsBatchArray of batches) {
                await Promise.all(paramsBatchArray.map(async (params) => {
                    const primaryResourceTarget = {
                        resourceId: '',
                        role: params.serverRole || Roles.Server.User,
                        resourceType: ServerInviteResourceType,
                        primary: true
                    };
                    return await withOperationLogging(async () => createAndSendInvite({
                        target: params.email,
                        inviterId: context.userId,
        ...
        async streamInviteBatchCreate(_parent, args, context) {
            ...
            for (const paramsBatchArray of batches) {
                await Promise.all(paramsBatchArray.map(async (params) => {
                    return await withOperationLogging(async () => createProjectInvite({
                        input: params,
                        inviterId: context.userId,
                        inviterResourceAccessRules: context.resourceAccessRules
                    }), {
        ...
        async streamInviteUse(_parent, args, ctx) {
            ...
            try {
                await withOperationLogging(async () => await useProjectInvite(args, ctx.userId, ctx.resourceAccessRules), {
                    logger,
                    operationName: 'streamInviteUse',
                    operationDescription: 'Use a stream invite'
                });
            } catch (e) {
                if (await isHarmlessDuplicateUseError(e, ctx, projectId)) {
                    logger.info({ err: e }, 'streamInviteUse: swallowing duplicate-click InviteNotFoundError; user already has access');
                    return true;
                }
                throw e;
            }
        ...
        async streamInviteCancel(_parent, args, ctx) {
            ...
            await withOperationLogging(async () => await cancelInvite({
                inviteId,
                cancelerId: userId,
        ...
        async inviteDelete(_parent, args, ctx) {
            ...
            await withOperationLogging(async () => await deleteInvite(inviteId, ctx.userId), {
                logger,
    ...
    ProjectInviteMutations: {
        ...
        async batchCreate(_parent, args, ctx) {
            ...
            for (const batch of inputBatches) {
                await Promise.all(batch.map((i) => withOperationLogging(async () => await createProjectInvite({
                    input: {
                        ...i,
                        projectId
                    },
        ...
        async use(_parent, args, ctx) {
            ...
            try {
                await withOperationLogging(async () => await useProjectInvite(args.input, ctx.userId, ctx.resourceAccessRules), {
                    logger,
                    operationName: 'projectInviteUse',
                    operationDescription: 'Use a project invite'
                });
            } catch (e) {
                if (await isHarmlessDuplicateUseError(e, ctx, args.input?.projectId)) {
                    logger.info({ err: e }, 'projectInviteUse: swallowing duplicate-click InviteNotFoundError; user already has access');
                    return true;
                }
                throw e;
            }
...
//# sourceMappingURL=serverInvites.js.map