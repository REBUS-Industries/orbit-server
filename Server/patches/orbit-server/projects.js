...
import { getBatchedStreamCommentsFactory, getCommentLinksFactory, insertCommentLinksFactory, insertCommentsFactory } from '@/modules/comments/repositories/comments';
import { TokenResourceIdentifierType } from '@/modules/core/graph/generated/graphql';
import { Roles, Scopes } from '@/modules/core/helpers/mainConstants';
import { isResourceAllowed, throwIfNewResourceNotAllowed, throwIfResourceAccessNotAllowed, toProjectIdWhitelist } from '@/modules/core/helpers/token';
import { getBatchedStreamBranchesFactory, insertBranchesFactory } from '@/modules/core/repositories/branches';
import { deleteProjectCommitsFactory, getBatchedBranchCommitsFactory, getBatchedStreamCommitsFactory, insertBranchCommitsFactory, insertCommitsFactory, insertStreamCommitsFactory } from '@/modules/core/repositories/commits';
import { deleteProjectFactory, storeProjectFactory, storeProjectRoleFactory } from '@/modules/core/repositories/projects';
import { getServerInfoFactory } from '@/modules/core/repositories/server';
import { getStreamFactory, createStreamFactory, updateStreamFactory, revokeStreamPermissionsFactory, grantStreamPermissionsFactory, getOnboardingBaseStreamFactory, getUserStreamsPageFactory, getUserStreamsCountFactory, getStreamRolesFactory } from '@/modules/core/repositories/streams';
import { getUserFactory, getUsersFactory } from '@/modules/core/repositories/users';
import { createNewProjectFactory, deleteProjectAndCommitsFactory } from '@/modules/core/services/projects';
import { throwIfRateLimitedFactory } from '@/modules/core/utils/ratelimiter';
import { addOrUpdateStreamCollaboratorFactory, isStreamCollaboratorFactory, removeStreamCollaboratorFactory, validateStreamAccessFactory } from '@/modules/core/services/streams/access';
import { cloneStreamFactory } from '@/modules/core/services/streams/clone';
import { createStreamReturnRecordFactory, deleteStreamAndNotifyFactory, updateStreamAndNotifyFactory, updateStreamRoleAndNotifyFactory } from '@/modules/core/services/streams/management';
...
import { getProjectDbClient, getProjectReplicationDbs, getReplicationDbs, getValidDefaultProjectRegionKey } from '@/modules/multiregion/utils/dbSelector';
import { deleteAllResourceInvitesFactory, deleteInvitesByTargetFactory, deleteServerOnlyInvitesFactory, findInviteFactory, findUserByTargetFactory, insertInviteAndDeleteOldFactory, updateAllInviteTargetsFactory } from '@/modules/serverinvites/repositories/serverInvites';
...
import { filteredSubscribe, ProjectSubscriptions, UserSubscriptions } from '@/modules/shared/utils/subscriptions';
import { has } from 'lodash-es';
import { throwIfAuthNotOk } from '@/modules/shared/helpers/errorHelper';
import { withOperationLogging } from '@/observability/domain/businessLogging';
import { finalizeInvitedServerRegistrationFactory, finalizeResourceInviteFactory } from '@/modules/serverinvites/services/processing';
...
const buildFinalizeProjectInvite = (mainDb, emit) => finalizeResourceInviteFactory({
...
const deleteStreamAndNotify = async (projectId, userId, ctxLogger) => asMultiregionalOperation(({ allDbs, mainDb, emit }) => {
...
const resolvers = {
    ...
    ProjectMutations: {
        async batchDelete(_parent, args, ctx) {
            ...
            const results = await Promise.all(args.ids.map((id) => deleteStreamAndNotify(id, ctx.userId, ctx.log)));
            return results.every((res) => res === true);
        },
        async delete(_parent, { id: projectId }, { userId, resourceAccessRules, authPolicies, log: ctxLogger }) {
            throwIfResourceAccessNotAllowed({
        ...
        async createForOnboarding(_parent, _args, { userId, resourceAccessRules, log }) {
            return await asMultiregionalOperation(async ({ mainDb, emit, allDbs }) => {
        ...
        async update(_parent, { update }, { userId, resourceAccessRules, authPolicies, clearCache, log: ctxLogger }) {
            ...
            const res = await asMultiregionalOperation(async ({ mainDb, allDbs, emit }) => {
        ...
        async create(_parent, args, context) {
            ...
            const project = await asMultiregionalOperation(async ({ allDbs, mainDb, emit }) => {
        ...
        async updateRole(_parent, args, ctx) {
            ...
            await authorizeResolver(ctx.userId, projectId, Roles.Stream.Owner, ctx.resourceAccessRules);
            const logger = ctx.log.child({
                projectId,
                streamId: projectId //legacy
            });
            const ret = await withOperationLogging(async () => await updateStreamRoleAndNotify(args.input, ctx.userId, ctx.resourceAccessRules), {
                logger,
        ...
        async leave(_parent, args, context) {
            ...
            await withOperationLogging(async () => await removeStreamCollaborator(id, userId, userId, context.resourceAccessRules), {
                logger,
    ...
    User: {
        async projects(_parent, args, ctx) {
            ...
            // ORBIT custom — server admins with ADMIN_OVERRIDE_ENABLED see EVERY
            // project in their Projects list, not just the ones they were
            // explicitly invited to. The flag is honoured by
            // getUserStreamsQueryBaseFactory which skips the ACL-required join
            // condition. Without this, admin override only opens individual
            // project URLs, not the listing.
            const serverAdminBypass = !!(ctx.role === Roles.Server.Admin && adminOverrideEnabled());
            const [totalCount, visibleCount, { cursor, streams }] = await Promise.all([
                getUserStreamsCount({
                    userId: ctx.userId,
                    forOtherUser: false,
                    searchQuery: args.filter?.search || undefined,
                    withRoles: (args.filter?.onlyWithRoles || []),
    ...
    Subscription: {
        userProjectsUpdated: {
            subscribe: filteredSubscribe(UserSubscriptions.UserProjectsUpdated, (payload, _args, ctx) => {
        ...
        projectUpdated: {
            subscribe: filteredSubscribe(ProjectSubscriptions.ProjectUpdated, async (payload, args, ctx) => {
                if (args.id !== payload.projectUpdated.id)
                    return false;
                throwIfResourceAccessNotAllowed({
