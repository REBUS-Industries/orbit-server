import { clamp, groupBy, has, isNaN, isNull, isObjectLike, isUndefined, mapValues, omit, omitBy, reduce, toNumber, keyBy } from 'lodash-es';
import { Streams, StreamAcl, StreamFavorites, knex, Users, StreamCommits, Commits, ServerAcl } from '@/modules/core/dbSchema';
import { InvalidArgumentError, LogicError } from '@/modules/shared/errors';
import { Roles } from '@/modules/core/helpers/mainConstants';
import { ProjectRecordVisibility } from '@/modules/core/helpers/types';
import { DiscoverableStreamsSortType, SortDirection } from '@/modules/core/graph/generated/graphql';
import { decodeCompositeCursor, decodeCursor, encodeCompositeCursor, encodeCursor } from '@/modules/shared/helpers/dbHelper';
import dayjs from 'dayjs';
import cryptoRandomString from 'crypto-random-string';
import { isProjectCreateInput, mapGqlToDbProjectVisibility } from '@/modules/core/helpers/project';
...
const tables = {
    streams: (db) => db(Streams.name),
    streamAcl: (db) => db(StreamAcl.name),
    streamCommits: (db) => db(StreamCommits.name),
    streamFavorites: (db) => db(StreamFavorites.name)
};
/**
 ...
 */
export const getStreamsFactory = (deps) => async (streamIds, options = {}) => {
    const { userId, trx } = options;
    if (!streamIds?.length)
        throw new InvalidArgumentError('Empty stream IDs');
 ...
 */
export const getStreamFactory = (deps) => async (params, options) => {
...
export const getCommitStreamsFactory = (deps) => async (params) => {
    const { commitIds, userId } = params;
    if (!commitIds?.length)
        return [];
    const q = tables
        .streamCommits(deps.db)
        .select([...Streams.cols, StreamCommits.col.commitId])
...
export const getCommitStreamFactory = (deps) => async (params) => {
    const { commitId } = params;
    if (!commitId)
        throw new InvalidArgumentError('Invalid commit ID');
 ...
 */
const getFavoritedStreamsQueryBaseFactory = (deps) => (userId, streamIdWhitelist) => {
    if (!userId)
 ...
 */
export const getFavoritedStreamsPageFactory = (deps) => async (params) => {
    ...
    if (cursor)
        query.andWhere(StreamFavorites.col.cursor, '<', cursor);
 ...
 */
export const getFavoritedStreamsCountFactory = (deps) => async (userId, streamIdWhitelist) => {
    const query = getFavoritedStreamsQueryBaseFactory(deps)(userId, streamIdWhitelist);
    query.count();
 ...
 */
export const setStreamFavoritedFactory = (deps) => async (params) => {
 ...
 */
export const getBatchUserFavoriteDataFactory = (deps) => async (params) => {
 ...
 */
export const getBatchStreamFavoritesCountsFactory = (deps) => async (streamIds) => {
    const query = tables
        .streamFavorites(deps.db)
        .columns([
 ...
 */
export const canUserFavoriteStreamFactory = (deps) => async (params) => {
    ...
    const query = tables
        ...
        .andWhere(function () {
        this.where(Streams.col.visibility, ProjectRecordVisibility.Public).orWhereNotNull(StreamAcl.col.resourceId);
    })
        .limit(1);
 ...
 */
export const getOwnedFavoritesCountByUserIdsFactory = (deps) => async (userIds) => {
    const query = tables
        .streamAcl(deps.db)
        .select([
 ...
 */
export const getStreamRolesFactory = (deps) => async (userId, streamIds) => {
    const q = tables
        .streams(deps.db)
        .select([
        Streams.col.id,
    ...
    const results = await q;
    return mapValues(keyBy(results, (r) => r.id), (v) => v.role);
};
const buildDiscoverableStreamsBaseQueryFactory = (deps) => (params) => {
    const q = tables
        .streams(deps.db)
        .select(Streams.cols)
...
const decodeDiscoverableStreamsCursor = (sortType, cursor) => {
...
export const encodeDiscoverableStreamsCursor = (sortType, retrievedStreams, previousCursor) => {
    ...
    switch (sortType) {
        ...
        case DiscoverableStreamsSortType.FavoritesCount: {
            ...
            const previousOffset = decodedPreviousCursor || 0;
 ...
 */
export const countDiscoverableStreamsFactory = (deps) => async (params) => {
    const q = buildDiscoverableStreamsBaseQueryFactory(deps)(params);
    q.clearSelect();
    q.count();
    const [res] = await q;
    return parseInt(res.count);
 ...
 */
export const getDiscoverableStreamsPageFactory = (deps) => async (params) => {
...
export const getStreamsCollaboratorCountsFactory = (deps) => async ({ streamIds, type }) => {
    if (!streamIds.length)
        return {};
    const q = tables
        .streamAcl(deps.db)
 ...
 */
export const getStreamsCollaboratorsFactory = (deps) => async ({ streamIds }) => {
    if (!streamIds.length)
        return {};
    const q = tables
        .streamAcl(deps.db)
 ...
 */
export const getStreamCollaboratorsFactory = (deps) => async (streamId, type, options) => {
    const { limit } = options || {};
    const q = tables
        .streamAcl(deps.db)
        .select([
 ...
 */
const getUserStreamsQueryBaseFactory = (deps) => ({ userId, searchQuery, forOtherUser, ownedOnly, withRoles, streamIdWhitelist, workspaceId, onlyWithActiveSsoSession, personalOnly, includeImplicitAccess, serverAdminBypass }) => {
    const query = tables.streams(deps.db).leftJoin(StreamAcl.name, (j1) => {
        j1.on(StreamAcl.col.resourceId, Streams.col.id).andOnVal(StreamAcl.col.userId, userId);
    });
    // ORBIT custom — server admins with ADMIN_OVERRIDE_ENABLED see every project
    // in their listing, not just ones they were explicitly invited to. Skip the
    // ACL-required filter and the implicit-access workspace logic; other
    // filters (search/workspace/withRoles) still apply.
    if (serverAdminBypass) {
        // intentionally no whereNotNull on StreamAcl.col.role — every stream
        // is returned, with the LEFT JOIN producing null role for projects
        // the admin isn't an explicit member of (the resolver doesn't care).
    }
        ...
        query
            ...
            j2.on(WorkspaceAcl.col.workspaceId, Streams.col.workspaceId).andOnVal(WorkspaceAcl.col.userId, userId);
        })
            .andWhere((w1) => {
            w1.whereNotNull(StreamAcl.col.role).orWhere((w2) => {
                ...
                w2.whereNotNull(WorkspaceAcl.col.role).andWhere((w2) => {
                    w2.andWhere(WorkspaceAcl.col.role, Roles.Workspace.Admin).orWhere((w4) => {
                        w4.where(WorkspaceAcl.col.role, '!=', Roles.Workspace.Guest).andWhereNot(Streams.col.visibility, ProjectRecordVisibility.Private);
                    });
                });
            });
        });
    ...
    if (onlyWithActiveSsoSession) {
        query
            .leftJoin('workspace_sso_providers', 'workspace_sso_providers.workspaceId', Streams.col.workspaceId)
            .leftJoin('user_sso_sessions', function () {
            this.on('user_sso_sessions.userId', '=', StreamAcl.col.userId).andOn('workspace_sso_providers.providerId', '=', 'user_sso_sessions.providerId');
        })
            .andWhere(function () {
            this.whereRaw('workspace_sso_providers."providerId" is not null and user_sso_sessions."validUntil" > now()').orWhere('workspace_sso_providers.providerId', 'is', null);
        });
    }
...
function addSortByProjectRoleCondition(query) {
    return query.orderByRaw(`CASE WHEN stream_acl."role" = '${Roles.Stream.Owner}' THEN 1 WHEN stream_acl."role" = '${Roles.Stream.Contributor}' THEN 2 WHEN stream_acl."role" = '${Roles.Stream.Reviewer}' THEN 3 end asc`);
}
/**
 * Get streams the user is a collaborator on
 */
export const getUserStreamsPageFactory = (deps) => async (params) => {
    ...
    const decodedCursor = decodeCompositeCursor(cursor, (c) => isObjectLike(c) && has(c, 'id') && has(c, 'updatedAt'));
    if (decodedCursor) {
 ...
 */
export const getUserStreamsCountFactory = (deps) => async (params) => {
    const query = getUserStreamsQueryBaseFactory(deps)(params);
    const countQuery = query.count();
    const [res] = await countQuery;
    return parseInt(res.count);
};
export const createStreamFactory = (deps) => async (input) => {
    const { name, description, updatedAt, createdAt, allowPublicComments, id } = input;
    let visibility;
    if (isProjectCreateInput(input)) {
        visibility = mapGqlToDbProjectVisibility(input.visibility || (input.workspaceId ? 'WORKSPACE' : 'PRIVATE'));
    }
    else {
        visibility =
    ...
    const newStream = insertResults[0];
    return newStream;
};
export const getUserStreamCountsFactory = (deps) => async (params) => {
...
export const getStreamsSourceAppsFactory = (deps) => async (streamIds) => {
    if (!streamIds?.length)
        return {};
    const q = tables
        .streams(deps.db)
    ...
    const results = await q;
    const mappedToSets = reduce(results, (result, item) => {
        const set = result[item.id] || new Set();
        if (item.sourceApplication?.length)
...
const isProjectUpdateInput = (i) => has(i, 'visibility');
export const updateStreamFactory = (deps) => async (update) => {
    const { id: streamId } = update;
    if (!update.name)
        update.name = null; // to prevent saving name ''
    const validUpdate = omitBy(update, (v) => isNull(v) || isUndefined(v));
    if (isProjectUpdateInput(update)) {
        if (has(update, 'visibility')) {
            validUpdate.visibility = mapGqlToDbProjectVisibility(update.visibility || 'PRIVATE');
        }
    }
    else {
    ...
    else if (has(validUpdate, 'allowPublicComments') &&
...
export const updateProjectFactory = ({ db }) => async ({ projectUpdate }) => {
    const [updatedStream] = await tables
        .streams(db)
        .returning('*')
...
export const upsertProjectRoleFactory = ({ db }) => async ({ projectId, userId, role }) => {
...
export const grantStreamPermissionsFactory = (deps) => async (params) => {
 ...
 */
export const markOnboardingBaseStreamFactory = (deps) => async (streamId, version, updatedAt) => {
 ...
 */
export const getOnboardingBaseStreamFactory = (deps) => async (version) => {
    const q = tables
        .streams(deps.db)
        .select(Streams.cols)
 ...
 */
export const legacyGetStreamsFactory = (deps) => async ({ cursor, limit, orderBy, visibility, searchQuery, streamIdWhitelist, workspaceIdWhitelist, offset, publicOnly, userId }) => {
    const query = tables.streams(deps.db);
    if (searchQuery) {
        const whereFunc = function () {
            this.where('streams.name', 'ILIKE', `%${searchQuery}%`).orWhere('streams.description', 'ILIKE', `%${searchQuery}%`);
        };
    ...
    if (visibility && visibility !== 'all') {
        if (![
            ...
            throw new LogicError('Stream visibility should be either private, public, workspace or all');
    ...
    if (cursor)
        query.where(columnName, order === 'desc' ? '<' : '>', cursor);
...
export const getUserDeletableStreamsFactory = (deps) => async (id) => {
 ...
 */
export const getImplicitUserProjectsCountFactory = (deps) => async (params) => {
    const q = tables
        .streams(deps.db)
        .select(knex.raw('COUNT(??) as count', [Streams.col.id]))
        .leftJoin(StreamAcl.name, (j) => {
        j.on(StreamAcl.col.resourceId, Streams.col.id).andOnVal(StreamAcl.col.userId, params.userId);
    })
        .leftJoin(WorkspaceAcl.name, (j) => {
        j.on(WorkspaceAcl.col.workspaceId, Streams.col.workspaceId).andOnVal(WorkspaceAcl.col.userId, params.userId);
    })
        .where((w) => {
 ...
 */
export const getExplicitProjects = (deps) => async ({ limit, cursor, filter: { userId, workspaceId } }) => {
    if (!userId && !workspaceId)
        throw new LogicError('A filter must be provided');
    ...
    if (userId) {
        q.join(StreamAcl.name, (j) => {
            j.on(StreamAcl.col.resourceId, Streams.col.id).andOnVal(StreamAcl.col.userId, userId);
        });
    }
    if (cursor) {
